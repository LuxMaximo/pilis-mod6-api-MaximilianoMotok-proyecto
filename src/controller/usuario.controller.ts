...
const createToken = (user: User) => {
    ...
    const token = jwt.sign({ id: user.id, email: user.email }, jwtSecret, { expiresIn: '10s' });
...
export const updateUser = async (req: Request, res: Response) => {
    ...
    try {
        const user = await Usuario.findOneBy({ id: parseInt(id) });
...
export const refresh = async (req: Request, res: Response): Promise<any> => {
    ...
    //includes verifica si esta en el array
